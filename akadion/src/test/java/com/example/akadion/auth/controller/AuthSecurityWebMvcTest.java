...
import com.example.akadion.auth.config.SecurityConfig;
...
import com.example.akadion.admin.controller.AdminController;
...
class AuthSecurityWebMvcTest {
    ...
    void adminCanApproveUsersWithCsrf() throws Exception {
        when(userRepository.findByIdKeycloak("sub-admin")).thenReturn(Optional.of(user("sub-admin", "ADMIN", "ACTIV")));
        when(adminUserService.approveUser(5L)).thenReturn(new ActionResponseDto("ok"));
    ...
    void adminCanRejectUsersWithCsrf() throws Exception {
        when(userRepository.findByIdKeycloak("sub-admin")).thenReturn(Optional.of(user("sub-admin", "ADMIN", "ACTIV")));
        when(adminUserService.rejectUser(7L)).thenReturn(new ActionResponseDto("respins"));
