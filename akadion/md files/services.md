...
## 14. `AuditLogService`
- **Scop:** Scrie și citește jurnalul de audit (`audit_log`) — cine a făcut o acțiune sensibilă (aprobare/respingere/dezactivare cont, creare/editare/ștergere curs-săptămână-document), când, și cu ce valori înainte/după. Documentație completă, cu exemple de cod și motivația fiecărei decizii de design: `explicatie_audit_jurnal.md`.
- **Scriere — `inregistreaza(numeTabel, idInregistrare, operatie, valoriVechi, valoriNoi)`:** `@Transactional(propagation = Propagation.MANDATORY)` — refuză să scrie dacă nu e apelată dintr-o tranzacție deja deschisă (garantează că logul nu poate exista fără ca operațiunea de bază să fi reușit efectiv). Autorul (`utilizator`) nu vine niciodată din request — se citește din `SecurityContext` prin `AuditorAware` (același mecanism ca `createdBy`/`updatedBy`, vezi `configurari.md` §2.1); `"system"` dacă nu există utilizator logat (ex: seeding la pornire). Apelată din `AdminUserService`, `CompleteProfileService`, `UserProfileService`, `CursService`, `SaptamanaService`, `DocumentService`.
- **Citire — `getAuditLog(Pageable)`:** `@Transactional(readOnly = true)`. Rezolvă ID-urile Keycloak ale autorilor la nume/email afișabile printr-o singură interogare batch (`UserRepository.findByIdKeycloakIn`), nu una per rând — evită N+1 query la paginare. Rol de securitate: destinat exclusiv accesului `ADMIN`, expus prin `GET /api/admin/audit-log` (vezi `api-controllers.md` §1.7).
- **Reguli de design notabile:** cascadele (ex: ștergerea unei săptămâni care șterge și documentele ei) generează un singur log pe acțiunea sursă, nu unul per efect secundar; auditul unui upload de document reflectă succesul local (MinIO+DB), independent de răspunsul RAG. Detalii și exemple de cod: `explicatie_audit_jurnal.md` §6.
