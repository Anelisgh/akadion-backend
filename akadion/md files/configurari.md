...
> Actualizat 2026-08-13. Restructurare package-by-feature: unele clase de config au rămas cross-cutting la rădăcina `com.example.akadion.config` (cele de mai jos), altele s-au mutat în pachetul de config al feature-ului propriu — `auth/config/` (`SecurityConfig`, `OAuth2ClientConfig`, `WebMvcConfig`) și `curs/config/` (`MinioConfig`, `MinioInitializer`).
...
Aplicația este un monolit modular construit în Spring Boot 3. De la restructurarea din 2026-08-13, pachetele sunt organizate **pe feature** (nu mai e layered architecture clasică):
- **`admin/`, `akychat/`, `auth/`, `curs/`, `quiz/`** — câte un pachet per domeniu de business, fiecare cu subpachete interne `controller/`, `service/`, `repository/`, `dto/`, `entity/` (unde e cazul; `admin/` e plat, fără subpachete).
- **`common/`** — `entity/`, `repository/`, `dto/` partajate între feature-uri (`User`, `Rol`, `StareCont`, `NumeRol`, `NumeStareCont`, `UserRepository` etc.).
- **`config/`** — bean-uri de configurare cu adevărat globale, cross-cutting (vezi mai jos).
- **`exception/`** — ierarhia de excepții custom + `GlobalExceptionHandler`, rămasă cross-cutting.
