...
2. `PATCH /users/{id}/approve`
   ...
   - **Excepții:** `InvalidUserStateException` dacă starea utilizatorului nu e `PENDING`. `UserNotFoundException` dacă id-ul nu există. `ForbiddenOperationException` dacă ținta este ADMIN.
...
5. `PUT /{id}`
   ...
   - **Excepții:** `AccesInterzisException` (dacă profesorul logat nu este owner).
...
## 7. `StudentController`
- **Path de bază:** `/api/student`
- **Securitate:** Necesită rol `STUDENT`.
